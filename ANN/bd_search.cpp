...
void ANNbd_shrink::ann_search(ANNdist box_dist, ANNQueryParam *param) const
{
...
//	if (ANNmaxPtsVisited != 0 && ANNptsVisited > ANNmaxPtsVisited) return;
	...
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(param->ANNkdQ)) {				// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(param->ANNkdQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_search(inner_dist, param);	// search inner child first
		child[ANN_OUT]->ann_search(box_dist, param);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_search(box_dist, param);	// search outer child first
		child[ANN_IN]->ann_search(inner_dist, param);	// ...then outer child
	}
