...
void ANNbd_shrink::ann_FR_search(ANNdist box_dist, ANNFrQueryParam *param) const
{
	...
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(param->ANNkdFRQ)) {			// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist)ANN_SUM(inner_dist, bnds[i].dist(param->ANNkdFRQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_FR_search(inner_dist, param);// search inner child first
		child[ANN_OUT]->ann_FR_search(box_dist, param);// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_FR_search(box_dist, param);// search outer child first
		child[ANN_IN]->ann_FR_search(inner_dist, param);// ...then outer child
	}
