	...
	double				eps) const			// error bound (ignored)
{
	...
	ANNPriQueryParam param;
	param.ANNprMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating ops

	param.ANNprDim = dim;						// copy arguments to static equivs
	
	//ANNprQ = q;				//by Jones.
	param.ANNprQ = q;			 //by Jones.

	param.ANNprPts = pts;
	//ANNptsVisited = 0;					// initialize count of points visited

	param.ANNprPointMK = new ANNmin_k(k);		// create set for closest k points

										// distance to root box
	ANNdist box_dist = annBoxDistance(q,
				bnd_box_lo, bnd_box_hi, dim);

	param.ANNprBoxPQ = new ANNpr_queue(n_pts);// create priority queue for boxes
	param.ANNprBoxPQ->insert(box_dist, root); // insert root in priority queue
	
	while (param.ANNprBoxPQ->non_empty()
		...
		) 
	{
		...
		param.ANNprBoxPQ->extr_min(box_dist, (void *&)np);

		ANN_FLOP(2)						// increment floating ops
		if (box_dist*param.ANNprMaxErr >= param.ANNprPointMK->max_key())
			break;

		np->ann_pri_search(box_dist, &param);	// search this subtree.
	}

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = param.ANNprPointMK->ith_smallest_key(i);
		nn_idx[i] = param.ANNprPointMK->ith_smallest_info(i);
	}

	delete param.ANNprPointMK;				// deallocate closest point set
	delete param.ANNprBoxPQ;					// deallocate priority queue
}
...
void ANNkd_split::ann_pri_search(ANNdist box_dist, ANNPriQueryParam *param) const
{
	ANNdist new_dist;					// distance to child visited later
										// distance to cutting plane
	ANNcoord cut_diff = param->ANNprQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		ANNcoord box_diff = cd_bnds[ANN_LO] - param->ANNprQ[cut_dim];
		...
		if (child[ANN_HI] != KD_TRIVIAL)// enqueue if not trivial
			param->ANNprBoxPQ->insert(new_dist, child[ANN_HI]);
										// continue with closer child
		child[ANN_LO]->ann_pri_search(box_dist, param);
	}
	else {								// right of cutting plane
		ANNcoord box_diff = param->ANNprQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		new_dist = (ANNdist) ANN_SUM(box_dist,
		...
		if (child[ANN_LO] != KD_TRIVIAL)// enqueue if not trivial
			param->ANNprBoxPQ->insert(new_dist, child[ANN_LO]);
										// continue with closer child
		child[ANN_HI]->ann_pri_search(box_dist, param);
	}
...
void ANNkd_leaf::ann_pri_search(ANNdist box_dist, ANNPriQueryParam *param) const
{
	...
	min_dist = param->ANNprPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = param->ANNprPts[bkt[i]];			// first coord of next data point
		qq = param->ANNprQ;					// first coord of query point
		dist = 0;

		for (d = 0; d < param->ANNprDim; d++) {
		...
		if (d >= param->ANNprDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			param->ANNprPointMK->insert(dist, bkt[i]);
			min_dist = param->ANNprPointMK->max_key();
		}
	}
