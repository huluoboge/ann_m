...
int ANNkd_tree::annkFRSearch(
	...
	double				eps) const			// the error bound
{
	ANNFrQueryParam queryParam;		// Jones.

	queryParam.ANNkdFRDim = dim;					// copy arguments to static equivs
	
	//ANNkdFRQ = q;
	
	queryParam.ANNkdFRQ = q;

	queryParam.ANNkdFRSqRad = sqRad;
	queryParam.ANNkdFRPts = pts;
	//ANNkdFRPtsVisited = 0;				// initialize count of points visited
	queryParam.ANNkdFRPtsInRange = 0;				// ...and points in the range

	queryParam.ANNkdFRMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating op count

	queryParam.ANNkdFRPointMK = new ANNmin_k(k);	// create set for closest k points
										// search starting at the root
	root->ann_FR_search(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim), &queryParam);

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		if (dd != NULL)
			dd[i] = queryParam.ANNkdFRPointMK->ith_smallest_key(i);
		if (nn_idx != NULL)
			nn_idx[i] = queryParam.ANNkdFRPointMK->ith_smallest_info(i);
	}

	delete queryParam.ANNkdFRPointMK;				// deallocate closest point set
	return queryParam.ANNkdFRPtsInRange;			// return final point count
}
...
void ANNkd_split::ann_FR_search(ANNdist box_dist, ANNFrQueryParam *param) const
{
										// check dist calc term condition
	//if (ANNmaxPtsVisited != 0 && ANNkdFRPtsVisited > ANNmaxPtsVisited) return;

										// distance to cutting plane
	ANNcoord cut_diff = param->ANNkdFRQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		child[ANN_LO]->ann_FR_search(box_dist, param);// visit closer child first

		ANNcoord box_diff = cd_bnds[ANN_LO] - param->ANNkdFRQ[cut_dim];
		...
		if (box_dist * param->ANNkdFRMaxErr <= param->ANNkdFRSqRad)
			child[ANN_HI]->ann_FR_search(box_dist, param);

	}
	else {								// right of cutting plane
		child[ANN_HI]->ann_FR_search(box_dist, param);// visit closer child first

		ANNcoord box_diff = param->ANNkdFRQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		box_dist = (ANNdist) ANN_SUM(box_dist,
		...
		if (box_dist * param->ANNkdFRMaxErr <= param->ANNkdFRSqRad)
			child[ANN_LO]->ann_FR_search(box_dist, param);
...
void ANNkd_leaf::ann_FR_search(ANNdist box_dist, ANNFrQueryParam *param) const
{
	...
	for (int i = 0; i < n_pts; i++) {	// check points in bucket
		...
		pp = param->ANNkdFRPts[bkt[i]];		// first coord of next data point
		qq = param->ANNkdFRQ;					// first coord of query point
		dist = 0;

		for (d = 0; d < param->ANNkdFRDim; d++) {
			ANN_COORD(1)				// one more coordinate hit
			ANN_FLOP(5)					// increment floating ops

			t = *(qq++) - *(pp++);		// compute length and adv coordinate
										// exceeds dist to k-th smallest?
			if ((dist = ANN_SUM(dist, ANN_POW(t))) > param->ANNkdFRSqRad) {
				break;
			}
		}

		if (d >= param->ANNkdFRDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			param->ANNkdFRPointMK->insert(dist, bkt[i]);
			param->ANNkdFRPtsInRange++;				// increment point count
		}
	}
